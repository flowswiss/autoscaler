...
func NewAttachedElasticIPService(client flowgo.Client, deviceID int) AttachedElasticIPService {
	return AttachedElasticIPService{client: client, deviceID: deviceID}
}

func (a AttachedElasticIPService) List(ctx context.Context, cursor flowgo.Cursor) (list ElasticIPList, err error) {
...
func getSpecificElasticIPPath(id int) string {
	return flowgo.Join(elasticIPsSegment, id)
}

func getAttachedElasticIPsPath(deviceID int) string {
	return flowgo.Join(getSpecificDevicePath(deviceID), attachedElasticIPsSegment)
}

func getSpecificAttachedElasticIPPath(deviceID, elasticIPID int) string {
	return flowgo.Join(getAttachedElasticIPsPath(deviceID), elasticIPID)
}
